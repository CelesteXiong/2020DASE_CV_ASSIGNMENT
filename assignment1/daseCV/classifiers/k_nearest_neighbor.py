...
#                 pass
    ...
    def compute_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            t_dist = (self.X_train - X[i])*(self.X_train - X[i])
   
            l2_dist = np.sqrt(np.sum(t_dist, axis=1))
...
#             pass
    ...
    def compute_distances_no_loops(self, X):
        ...
        tr = np.sum(self.X_train*self.X_train, axis =1)
        te = np.sum(X*X, axis =1)
        dists = np.sqrt(tr + np.reshape(te, (X.shape[0], 1)) - 2*dot)
...
#         pass
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            sort_dists = np.argsort(dists[i, :], axis=0)
            top_k = sort_dists[:k]
